...
local function secure_type(val)
    local success, result = pcall(type, val)
    if success then
        if result == "table" then
            local is_proxy, proxy_val = pcall(function() return rawget(val, "__value") end)
            if is_proxy and proxy_val then
                return type(proxy_val)
            end
        end
        return result
    end
    ...
    if mt and mt.__type then
        local mt_success, mt_res = pcall(function() return mt.__type end)
        if mt_success and type(mt_res) == "string" then 
            return mt_res 
        end
    end
    return "userdata"
...
local function aB()
    local watermark = "-- Ts file is generated using sks https://discord.gg/vGzAYErbYs\n\n"
    return watermark .. table.concat(t.output, "\n")
end
...
local function bl(bm)
    ...
    bi.__index = function(b2, b4)
        if b4 == F or b4 == "__proxy_id" or b4 == v or b4 == "__value" or b4 == "__temp_path" then
            return rawget(b2, b4)
        end
        return bl(0)
    end
...
bk = function(aS, bw)
    ...
    bi.__call = function(self, bz, ...)
        ...
        local function isJunkVariable(varName, expressionValue)
            if #varName > 15 and (varName:match("^[I1l]+$") or varName:match("^b[A-Za-z0-9_]+$")) then
                if expressionValue == "{}" or expressionValue:match("^%d+$") then
                    return true
                end
            end
            return false
            ...
            if j(b5) == "table" and not G(b5) and L == bF then
                ...
                for bG, aF in D(b5) do
                    ...
                    if bG == bE and #bJ > 0 then
                        ...
                        table.insert(b8, bd .. " = " .. bL:gsub("function%(%)", "function(...)") .. "\n" .. table.concat(bM, "\n") .. "\n" .. bc .. "end")
                    elseif bG == bE then
...
loadstring = function(al, eu)
    ...
    if cI:match("^https?://") then
        ...
        at(string.format('local object = loadstring(game:HttpGet("%s"))()', cI))
        return function()
            return ez
        end
    end
...
function eE:process_statement(eF)
    ...
    if type(eF) == "table" and eF.text then
        return tostring(eF.text)
    end
    return tostring(eF or "nil")
end
...
if not table.date then
    table.date = function(t, method)
...
task.spawn = function(f, ...)
    waitCounter = 0 
    local status, err = pcall(oldSpawn, f, ...)
    return status, err
end
